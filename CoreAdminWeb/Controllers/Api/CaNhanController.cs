...
public class CaNhanController : ControllerBase
{
    ...
    public async Task<IActionResult> GetMedicalData([FromQuery]string? maBenhNhan, [FromQuery]string? congTy, [FromQuery]DateTime? fromDate, [FromQuery]DateTime? toDate, [FromQuery]int offset = 0, [FromQuery]int limit = 10)
    {
        ...
        try
        {
            ...
            {
                where += " WHERE sksk.ma_benh_nhan = " + maBenhNhan;
            }
            ...
            var dataSql = @"
                ...
                " + where + @"
                GROUP BY sksk.id, sksk.ma_luot_kham, sksk.ngay_kham, ct.code, u.ma_benh_nhan, u.last_name, u.first_name, u.ngay_sinh, u.gioi_tinh,  
                ts.ten_benh, ts.tien_su_gia_dinh,
                tl.chieu_cao, tl.can_nang, tl.bmi, tl.mach, tl.huyet_ap,
                ck.kq_nk_tuan_hoan, ck.kq_nk_ho_hap, ck.kq_nk_tieu_hoa, ck.kq_nk_than_tiet_nieu, ck.kq_nk_noi_tiet,
                ck.kq_nk_co_xuong_khop, ck.kq_nk_than_kinh, ck.kq_nk_tam_than, ck.kq_ngoai_khoa,
