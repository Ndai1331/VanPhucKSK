...
public class KhamSucKhoeKQCLSController : ControllerBase
{
    ...
    [HttpGet("get-ket-qua")]
    public async Task<IActionResult> GetData([FromQuery] string? maLuotKham, [FromQuery] List<string>? maLuotKhams, [FromQuery] string? isAbnormal, [FromQuery] string? loaiCLS)
    {
    ...
    (kq.ten_loai_cls NOT IN (N'Huyết học') AND kq.ma_cls <> 'XN210')";
            ...
            if (!string.IsNullOrEmpty(loaiCLS))
            {
                whereClause += (string.IsNullOrEmpty(whereClause) ? " WHERE " : " AND ") + $" kq.loai_cls = {loaiCLS}";
            }
...
ORDER BY gd.ma_benh_nhan, gd.ma_luot_kham, gd.ord;
    ...
    public async Task<IActionResult> GetDetailData([FromQuery] string? maLuotKham, [FromQuery] List<string>? maLuotKhams, [FromQuery] string? maCls, [FromQuery] string? loaiCLS, [FromQuery] int? offset, [FromQuery] int? limit)
    {
        var response = new RequestHttpResponse<List<KetQuaCLSChiTietModel>>();
        try
        {
            ...
            if (maLuotKhams != null && maLuotKhams.Any())
            {
                whereClause += $" AND kq.ma_luot_kham IN ({string.Join(", ", maLuotKhams.Select((c, i) => $"@maLuotKham{i}"))}) ";
            }
            ...
            if (!string.IsNullOrEmpty(loaiCLS))
            {
                whereClause += " AND kq.loai_cls = @loaiCLS";
            }
...
ORDER BY
    ...
    g.gia_tri" + pagingClause;
            ...
            using (var dataCommand = _context.Database.GetDbConnection().CreateCommand())
            {
                ...
                if (!string.IsNullOrEmpty(loaiCLS))
                {
                    dataCommand.Parameters.Add(new SqlParameter("@loaiCLS", loaiCLS));
                }
                ...
                if (maLuotKhams != null && maLuotKhams.Any())
                {
                    for (int i = 0; i < maLuotKhams.Count; i++)
                    {
                        dataCommand.Parameters.Add(new SqlParameter($"@maLuotKham{i}", maLuotKhams[i]));
                    }
                }
                ...
                using (var reader = await dataCommand.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
                        var item = new KetQuaCLSChiTietModel
                        {
                            ...
                            ket_qua_chi_so = DataSetHelper.ReadString(reader, "ket_qua_chi_so"),
                            gia_tri = DataSetHelper.ReadString(reader, "gia_tri"),
