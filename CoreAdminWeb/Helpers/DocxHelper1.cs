...
public static class DocxHelper1
{
    public static void ReplaceText(this WordprocessingDocument doc, Dictionary<string, string> replacements)
    {
        ...
        foreach (var para in mainPart.Document.Descendants<Paragraph>())
        {
            ...
            {
                if (paraText.Contains(kv.Key))
                {
                    paraText = paraText.Replace(kv.Key, kv.Value ?? "");
                    needReplace = true;
                }
            }
            ...
            if (needReplace)
            {
                ...
                // Tạo 1 run mới với text đã replace
                var run = new Run(new Text(paraText));
    ...
    {
        var mainPart = doc.MainDocumentPart;
        if (mainPart == null) throw new Exception("Invalid document: no MainDocumentPart");
