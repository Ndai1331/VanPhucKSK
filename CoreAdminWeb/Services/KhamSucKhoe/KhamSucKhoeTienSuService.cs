...
namespace CoreAdminWeb.Services.KhamSucKhoe
{
    ...
    public class KhamSucKhoeTienSuService : IBaseDetailService<KhamSucKhoeTienSuModel>
    {
        ...
        /// <returns>A <see cref="RequestHttpResponse{T}"/> containing a list of <see cref="KhamSucKhoeTienSuModel"/> objects if
        /// the operation is successful. If the request fails, the response contains error details.</returns>
        public async Task<RequestHttpResponse<List<KhamSucKhoeTienSuModel>>> CreateAsync(List<dynamic> model)
        {
            if (model == null)
            {
                return new RequestHttpResponse<List<KhamSucKhoeTienSuModel>>
            ...
            // Check if all items in model are of type KhamSucKhoeTienSuModel
            bool allKhamSucKhoeTienSuModel = model.All(item => item is KhamSucKhoeTienSuModel);
            if (allKhamSucKhoeTienSuModel)
            {
                var typedList = model.Cast<KhamSucKhoeTienSuModel>().ToList();
                return await CreateAsync(typedList);
            }

            try
            {
                var response = await _httpClientService.PostAPIAsync<RequestHttpResponse<List<KhamSucKhoeTienSuModel>>>($"items/{_collection}?fields={Fields}", model);
        ...
        public async Task<RequestHttpResponse<bool>> UpdateAsync(List<dynamic> model)
        {
            ...
            try
            {
                var response = await _httpClientService.PatchAPIAsync<RequestHttpResponse<List<KhamSucKhoeTienSuModel>>>(
