...
namespace CoreAdminWeb.Services.KhamSucKhoe
{
    ...
    public class SoKhamSucKhoeService : IBaseDetailService<SoKhamSucKhoeModel>
    {
        ...
        /// <returns>A <see cref="RequestHttpResponse{T}"/> containing a list of <see cref="SoKhamSucKhoeModel"/> objects if
        /// the operation is successful. If the request fails, the response contains error details.</returns>
        public async Task<RequestHttpResponse<List<SoKhamSucKhoeModel>>> CreateAsync(List<dynamic> model)
        {
            if (model == null)
            {
                return new RequestHttpResponse<List<SoKhamSucKhoeModel>>
            ...
            // Check if all items in model are of type SoKhamSucKhoeModel
            bool allSoKhamSucKhoeModel = model.All(item => item is SoKhamSucKhoeModel);
            if (allSoKhamSucKhoeModel)
            {
                var typedList = model.Cast<SoKhamSucKhoeModel>().ToList();
                return await CreateAsync(typedList);
            }

            try
            {
                var response = await _httpClientService.PostAPIAsync<RequestHttpResponse<List<SoKhamSucKhoeModel>>>($"items/{_collection}?fields={Fields}", model);
        ...
        public async Task<RequestHttpResponse<bool>> UpdateAsync(List<dynamic> model)
        {
            ...
            try
            {
                var response = await _httpClientService.PatchAPIAsync<RequestHttpResponse<List<SoKhamSucKhoeModel>>>(
