    ...
    public class KhamSucKhoeCongTyService : IBaseGetService<KhamSucKhoeCongTyModel>
    {
        ...
        public async Task<RequestHttpResponse<List<KhamSucKhoeCongTyModel>>> GetAllAsync(string query, bool isPublic = false)
        {
            try
            {
                ...
                var response = isPublic ? await PublicRequestClient.GetAPIAsync<RequestHttpResponse<List<KhamSucKhoeCongTyModel>>>(url) : await _httpClientService.GetAPIAsync<RequestHttpResponse<List<KhamSucKhoeCongTyModel>>>(url);
