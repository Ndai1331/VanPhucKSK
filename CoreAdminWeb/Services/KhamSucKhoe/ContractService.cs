    ...
    public class ContractService : IBaseGetService<ContractModel>
    {
        ...
        public async Task<RequestHttpResponse<List<ContractModel>>> GetAllAsync(string query, bool isPublic = false)
        {
            try
            {
                ...
                var response = isPublic ? await PublicRequestClient.GetAPIAsync<RequestHttpResponse<List<ContractModel>>>(url) : await _httpClientService.GetAPIAsync<RequestHttpResponse<List<ContractModel>>>(url);
