...
namespace CoreAdminWeb.Services.KhamSucKhoe
{
    ...
    public class KhamSucKhoeTheLucService : IBaseDetailService<KhamSucKhoeTheLucModel>
    {
        ...
        /// <returns>A <see cref="RequestHttpResponse{T}"/> containing a list of <see cref="KhamSucKhoeTheLucModel"/> objects if
        /// the operation is successful. If the request fails, the response contains error details.</returns>
        public async Task<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>> CreateAsync(List<dynamic> model)
        {
            if (model == null)
            {
                return new RequestHttpResponse<List<KhamSucKhoeTheLucModel>>
            ...
            // Check if all items in model are of type KhamSucKhoeTheLucModel
            bool allKhamSucKhoeTheLucModel = model.All(item => item is KhamSucKhoeTheLucModel);
            if (allKhamSucKhoeTheLucModel)
            {
                var typedList = model.Cast<KhamSucKhoeTheLucModel>().ToList();
                return await CreateAsync(typedList);
            }

            try
            {
                var response = await _httpClientService.PostAPIAsync<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>>($"items/{_collection}?fields={Fields}", model);
        ...
        public async Task<RequestHttpResponse<bool>> UpdateAsync(List<dynamic> model)
        {
            ...
            try
            {
                var response = await _httpClientService.PatchAPIAsync<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>>(
