    ...
    public class KhamSucKhoeTheLucService : IBaseGetService<KhamSucKhoeTheLucModel>
    {
        ...
        public async Task<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>> GetAllAsync(string query, bool isPublic = false)
        {
            try
            {
                ...
                var response = isPublic ? await PublicRequestClient.GetAPIAsync<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>>(url) : await RequestClient.GetAPIAsync<RequestHttpResponse<List<KhamSucKhoeTheLucModel>>>(url);
        ...
        public async Task<RequestHttpResponse<KhamSucKhoeTheLucModel>> GetByIdAsync(string id)
        {
            ...
            try
            {
                var response = await RequestClient.GetAPIAsync<RequestHttpResponse<KhamSucKhoeTheLucModel>>($"items/{_collection}/{id}?fields={Fields}");
