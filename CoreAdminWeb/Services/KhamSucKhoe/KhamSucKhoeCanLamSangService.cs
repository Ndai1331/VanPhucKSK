...
namespace CoreAdminWeb.Services.KhamSucKhoe
{
    ...
    public class KhamSucKhoeCanLamSangService : IBaseDetailService<KhamSucKhoeCanLamSangModel>
    {
        ...
        /// <returns>A <see cref="RequestHttpResponse{T}"/> containing a list of <see cref="KhamSucKhoeCanLamSangModel"/> objects if
        /// the operation is successful. If the request fails, the response contains error details.</returns>
        public async Task<RequestHttpResponse<List<KhamSucKhoeCanLamSangModel>>> CreateAsync(List<dynamic> model)
        {
            if (model == null)
            {
                return new RequestHttpResponse<List<KhamSucKhoeCanLamSangModel>>
            ...
            // Check if all items in model are of type KhamSucKhoeCanLamSangModel
            bool allKhamSucKhoeCanLamSangModel = model.All(item => item is KhamSucKhoeCanLamSangModel);
            if (allKhamSucKhoeCanLamSangModel)
            {
                var typedList = model.Cast<KhamSucKhoeCanLamSangModel>().ToList();
                return await CreateAsync(typedList);
            }

            try
            {
                var response = await _httpClientService.PostAPIAsync<RequestHttpResponse<List<KhamSucKhoeCanLamSangModel>>>($"items/{_collection}?fields={Fields}", model);
        ...
        public async Task<RequestHttpResponse<bool>> UpdateAsync(List<dynamic> model)
        {
            ...
            try
            {
                var response = await _httpClientService.PatchAPIAsync<RequestHttpResponse<List<KhamSucKhoeCanLamSangModel>>>(
